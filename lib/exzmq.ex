...
defmodule Exzmq do
  ...
  @server_opts {}
    ...
    @doc "record_type owner: pid, fsm: term, identity: binary, mode: :'active'|:'active_once'|:'passive'"
    defmodule Socket do
      defstruct [:socket, owner: nil, fsm: nil, identity: "",
        # delivery mechanism
    ...
    defmodule Cargs do
      defstruct family: nil, address: nil, port: nil, tcpopts: nil, timeout: nil, failcnt: nil
    end
    ...
    def lb(transports, mqsstate = %Exzmq.Socket{transports: trans}) when is_list(transports) do
        ...
        %Exzmq.Socket{mqsstate | transports: trans1}
    end
    ...
    def remote_id_add(transport, remote_id, mqsstate = %Exzmq.Socket{remote_ids: rem_ids}) do
       %Exzmq.Socket{mqsstate | remote_ids: :orddict.store(remote_id, transport, rem_ids)}
    end

    def remote_id_del(transport, mqsstate = %Exzmq.Socket{remote_ids: rem_ids}) when is_pid(transport) do
      %Exzmq.Socket{mqsstate | remote_ids: :orddict.filter(fn(_key, value) -> value != transport end, rem_ids)}
    end
    
    def remote_id_del(remote_id, mqsstate = %Exzmq.Socket{remote_ids: rem_ids}) do
       %Exzmq.Socket{mqsstate |  remote_ids: :orddict.erase(remote_id, rem_ids)}
    end
    ...
    def transports_activate(transport, remote_id, mqsstate = %Exzmq.Socket{transports: transports}) do
      ...
      %Exzmq.Socket{mqsstate1 | transports: [transport|transports]}
    end
    ...
    def transports_deactivate(transport, mqsstate = %Exzmq.Socket{transports: transports}) do
      ...
      %Exzmq.Socket{mqsstate1 | transports: :lists.delete(transport, transports)}
    end
    ...
    def handle_call({:bind, :tcp, port, opts}, _from, mqsstate = %Exzmq.Socket{identity: identity}) do
        ...
        case Exzmq.Tcp.Socket.start_link(identity, port, tcpopts1) do
            {:ok, pid} ->
                ...
                {:reply, :ok, %Exzmq.Socket{mqsstate | listen_trans: listen}}
            reply ->
    ...
    def handle_call({:send, msg}, from, state) do
        case Exzmq.Socket.Fsm.check({:send, msg}, state) do
            {:queue, action} ->
                ...
                state1 = %Exzmq.Socket{state | send_q: state.send_q ++ [msg]}
                ...
                case action do
                    :return ->
                        ...
                        {:reply, :ok, state3}
                    :block ->
                        state3 = %Exzmq.Socket{state2 | pending_send: from}
    ...
    def handle_cast({:deliver_connect, transport, reply}, state = %Exzmq.Socket{connecting: connecting}) do
        case reply do
            ...
            {:error, reason} when reason == :eagain or 
                ...
                :erlang.send_after(3000, self(), {:reconnect, %Cargs{connect_args | failcnt: connect_args.failcnt + 1}})
                state2 = %Exzmq.Socket{state | connecting:  :orddict.erase(transport, connecting)}
                {:noreply, state2}
            _ ->
                state1 = %Exzmq.Socket{state | connecting:  :orddict.erase(transport, connecting)}
    ...
    def handle_cast({:deliver_close, transport}, state = %Exzmq.Socket{connecting: connecting}) do
        ...
        state3 = case :orddict.find(transport, connecting) do
                     {:ok, connect_args} ->
                         :erlang.send_after(3000, self(), {:reconnect, %Cargs{connect_args | failcnt:  0}})
                         %Exzmq.Socket{state1 |connecting: :orddict.erase(transport, connecting)}
                     _ ->
    ...
    def handle_info(:recv_timeout, %Exzmq.Socket{pending_recv:  {from, _}} = state) do
        ...
        state1 = %Exzmq.Socket{state | pending_recv: :none}
        {:noreply, state1}
    end
    ...
    defp do_connect(connect_args = %Cargs{family:  :tcp}, mqsstate = %Exzmq.Socket{identity:  identity}) do
        ...
        %Exzmq.Socket{mqsstate | connecting: connecting}
    end
    ...
    defp clear_send_queue(state = %Exzmq.Socket{send_q: [_msg], pending_send: from})
      when From != :none do
        ...
        %Exzmq.Socket{state1 | send_q: [], pending_send: :none}
    end

    defp clear_send_queue(state = %Exzmq.Socket{send_q:  [_msg|rest]}) do
        state1 = Exzmq.Socket.Fsm.work({:deliver_send, :abort}, state)
        clear_send_queue(%Exzmq.Socket{state1 | send_q: rest})
    end
    ...
    defp next_mode(%Exzmq.Socket{mode: :active_once} = state) do
        %Exzmq.Socket{state | mode: :passive}
    end
    ...
    defp handle_recv_2(timeout, from, 0, state) do
        ...
        state1 = %Exzmq.Socket{state | pending_recv: {from, ref}}
        {:noreply, state1}
    end
    ...
    defp queue(transport, value, mqsstate = %Exzmq.Socket{recv_q: q}) do
        ...
        mqsstate1 = %Exzmq.Socket{mqsstate | recv_q: q1}
    ...
    defp queue_close(transport, mqsstate = %Exzmq.Socket{recv_q: q}) do
        ...
        %Exzmq.Socket{mqsstate | recv_q: q1}
    end
    ...
    defp dequeue(mqsstate = %Exzmq.Socket{recv_q: q}) do
        ...
        case transports_while(&do_dequeue/2, q, :empty, mqsstate) do
            {{transport, value}, q1} ->
                mqsstate0 = %Exzmq.Socket{mqsstate | recv_q: q1}
    ...
    defp do_setopts({:identity, id}, mqsstate) do
        %Exzmq.Socket{mqsstate | identity: :erlang.iodata_to_binary(id)}
    end

    defp do_setopts({:active, :once}, mqsstate) do
        run_recv_q(%Exzmq.Socket{mqsstate | mode: :active_once})
    end

    defp do_setopts({:active, true}, mqsstate) do
        run_recv_q(%Exzmq.Socket{mqsstate | mode: :active})
    end
        
    defp do_setopts({:active, false}, mqsstate) do
        %Exzmq.Socket{mqsstate | mode: :passive}
    end
